...
// 为什么要进行这样的优化呢，是因为你虽然边变量发生了变化，但是你每次都这样使用扩展运算符，创建新的对象，也就是一次性递归到底生成新的对象，这样很是浪费内存如果在你需要管理数据的数据很庞大的情况下，使用map进行优化，这样即使你变量发生了变化，然后return出来的新的对象会尽可能的复用原来的属性，这样就节约了内存，这里进行了优化，combineReducer引入的路经也要发生改变，且在你组件里面得到的数据方式也会发生改变
// 未优化的版本
