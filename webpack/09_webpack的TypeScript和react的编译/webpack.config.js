...
module.exports = {
  entry: "./src/common_index.js", //这里是相对路经，入口的文件
  output: {
    ...
    rules: [
      ...
      },
      {
        ...
        use: {
          ...
          // 使用babel-loader处理ts文件的优点就是可以polyfill,但是缺点就是不会做类型检测，即使类型错了，还是会可以正常打包,如果使用ts-loader的话，就会对类型做检测，类型错误，就无法正常打包,所以建议在package.json文件中，先启动对tsc命令且--watch 然后在书写代码的时候就可视实时监听到ts文件的代码格式是否正确书写,打包的时候在使用babel-loader
          loader: "babel-loader",
        },
      },
    ],
